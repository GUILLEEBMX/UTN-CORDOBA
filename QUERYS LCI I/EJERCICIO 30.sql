...
SELECT DISTINCT C.COD_CLIENTE,C.NOMBRE,C.APELLIDO,L.COD_LOCALIDAD
FROM CLIENTES C ,LOCALIDADES L
WHERE C.COD_CLIENTE  = L.COD_LOCALIDAD;
...
WHERE L.COD_LOCALIDAD = C.COD_LOCALIDAD AND B.COD_BARRIO = C.COD_BARRIO AND  L.NOMBRE = 'CORDOBA' 
...
SELECT F.NRO_FACTURA, C.APELLIDO + ', ' + C.NOMBRE 'NOMBRE COMPLETO', DF.CANTIDAD, P.PRECIO, DF.CANTIDAD * P.PRECIO 'TOTAL'
FROM FACTURAS F, DETALLES_FACTURAS DF, PLANTAS P, CLIENTES C
